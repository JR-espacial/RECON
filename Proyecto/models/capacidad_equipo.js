...
module.exports =  class Capacidad_Equipo{
    ...
    static fetchAllPorcentajes(id_iteracion){
        return db.execute('SELECT CE.id_capacidad, horas_productivas, tiempo_perdido_pc, errores_registro_pc, overhead_pc, productivas_pc, operativos_pc, humano_pc, cmmi_pc FROM capacidad_equipo CE INNER JOIN iteracion I on CE.id_capacidad = I.id_capacidad WHERE id_iteracion =?', [id_iteracion]);
    }
    ...
    static updateCapacidadProductivas(id_capacidad, productivas_pc){
        return db.execute('UPDATE capacidad_equipo SET productivas_pc =? WHERE id_capacidad =?', [productivas_pc, id_capacidad]);
    }

    static updateCapacidadTiempoPerdido(id_capacidad, tiempo_perdido_pc){
        return db.execute('UPDATE capacidad_equipo SET tiempo_perdido_pc =? WHERE id_capacidad =?', [tiempo_perdido_pc, id_capacidad]);
    }

    static updateCapacidadErroresRegistro(id_capacidad, errores_registro_pc){
        return db.execute('UPDATE capacidad_equipo SET errores_registro_pc =? WHERE id_capacidad =?', [errores_registro_pc, id_capacidad]);
    }
    ...
    static updateCapacidadOperativos(id_capacidad, operativos_pc){
        return db.execute('UPDATE capacidad_equipo SET operativos_pc =? WHERE id_capacidad =?', [operativos_pc, id_capacidad]);
    }

    static updateCapacidadHumano(id_capacidad, humano_pc){
        return db.execute('UPDATE capacidad_equipo SET humano_pc =? WHERE id_capacidad =?', [humano_pc, id_capacidad]);
    }

    static updateCapacidadCMMI(id_capacidad, cmmi_pc){
        return db.execute('UPDATE capacidad_equipo SET cmmi_pc =? WHERE id_capacidad =?', [cmmi_pc, id_capacidad]);
    }
}